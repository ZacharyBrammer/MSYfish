...
def compute_pop_msy(
    ...
    outfile = outdir + species + '/msy_stocks_' + '%d' % f0.size + '_nfish_' + '%d' % nfish + '_mfish_' + \
        '%.4f' % np.max(f0) + '_rot_' + '%03d' % rotation + '_' + \
        '%d' % niter + '.nc'  # '_rec_' + '%.4f' % reprodper
    ...
    # gscale=.4
    gvar = growthCoef[1]*np.random.randn(N0)/2
    gvar[gvar <= -growthCoef[1]] = 0
    asympLeni = asympLen[0]+asympLen[1]*np.random.randn(N0)
    asympLeni[asympLeni < 0] = asympLen[0]

    Winf = (a*asympLeni**b)/1000  # asymptotic weight based on Winf=aasympLen^b
    Wmat = compute_wmat(asympLen[0], growthCoef[0], a, b, maxage)
    W0 = Winf*(1-np.exp(-(growthCoef[0]+gvar)*age0))**b

    bin_min = 0.1*(a*asympLen[0]**b)/1000*(1-np.exp(-(growthCoef[0])*age0))**b
    bin_max = 10*(a*asympLen[0]**b)/1000

    fish[0, :] = Winf*(1-np.exp(-(growthCoef[0]+gvar)*age))**b
    ...
    for ii in range(1, nyr):
                ...
                for mm in bio_order:
                    if mm == 0:
                       ...
                       # growth and reproduction
                        if (fish[ii, jj] < Wmat and avail_energy < metabolism):
                            ...
                            growth = pconsum*Winf[jj]*b*(growthCoef[0]+gvar[jj])*(fish[ii-1, jj]/Winf[jj])**(
                                1-1/b)*(1-(fish[ii-1, jj]/Winf[jj])**(1/b))
                    ...
                    if mm == 2:
                        ...
                        if ff[stock[jj]] > 0:
                            fishmort = scf * \
                                ff[stock[jj]] / \
                                (1+np.exp(-0.05*(fish[ii-1, jj]-Wfish[ii])))
                        ...
                        if caught < fishmprob and fish[ii, jj] > minsize:
                            ...
                            catch[ii, stock[jj], 1] = catch[ii,
                                                            stock[jj], 1]+fish[ii, jj]
                            consumption = 0
        ...
        for kk in range(0, nstocks):
            if nsx[nsx == kk].size > 0:
                ...
                [mort_bin[ii, :, kk], bin_edges] = np.histogram(
                    tempfish[0, nsx == kk], bins=bin_ranges)  # log
        ...
        for kk in range(0, nstocks):
            if nst[nst == kk].size > 0:
                ...
                [catch_bin[ii, :, kk], bin_edges] = np.histogram(
                    tempfish[0, nst == kk], bins=bin_ranges)  # log

        for kk in range(0, nstocks):
            for ll in range(0, nbins):
                ids = np.where(np.logical_and(np.logical_and(
                    fish[ii-1, :] >= bin_edges[ll], fish[ii-1, :] < bin_edges[ll+1]), stock == kk))
                if ids[0].shape[0] > 0:
        ...
        for kk in range(0, nstocks):
            ...
            [pop_bin[ii, :, kk], dumb] = np.histogram(
                fish[ii, stock == kk], bins=bin_ranges)  # log
            [age_bin[ii, :, kk], dumb] = np.histogram(
        ...
        if connectivity:
            ...
            for kk in range(0, nstocks):
                ...
                for mm in range(0, nstocks):
                    reprod_matrix[kk, mm] = conn_matrix[kk, mm] * \
                        reprod_tot[ii, kk]
                    # print(kk,mm,reprod_matrix[kk,mm])
                sigrec = np.random.lognormal(0., rvar)
                numrec_stock[kk] = int(
                    sigrec*np.sum(reprodper/1e5*reprod_matrix[:, kk]))
        ...
        # rep_scale=1/(1+np.exp(.01*(np.sum(fish[ii,:])-resourceAvail/12))) #density dependence in plankton
        ...
        if numrec > 0:
            ...
            newWinf = (a*newAsympLen**b)/1000
            newW0 = newWinf*(1-np.exp(-(growthCoef[0]+newgvar)*age0))**b
            newrec[ii, :] = newW0
...
def compute_wmat(asympLen, growthCoef, a, b, maxage):

    yrage = np.linspace(1, maxage, 120)
    Winf = (a*asympLen**b)/1000

    L = asympLen*(1-np.exp(-growthCoef*yrage))

    W = (a*L**b)/1000

    growth = Winf*b*growthCoef*(W/Winf)**(1-1/b)*(1-(W/Winf)**(1/b))
