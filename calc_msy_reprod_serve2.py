...
import os
from msy_stocks_rev11_26Mar21 import compute_pop_msy
import numpy as np
import pandas as pd
       ...
       #set file directories if needed
        if not (np.isnan(Linf) or np.isnan(K) or np.isnan(b) or np.isnan(maxage)):
            ...
            while fslap == 0:
                    ...
                    fslap = compute_pop_msy(outdir,f0,nstocks,nstocks,species,Linf,K,a,b,maxage,minsize,minrec,R,0,oo,1,0,0)
                    maxfish = maxfish +.01
            ...
            while stocktest == 0:
                ...
                for ii in range(0,fstep): #41 for complete runs
                    ...
                    slap = compute_pop_msy(outdir,f0,nstocks,nfish,species,Linf,K,a,b,maxage,minsize,minrec,R,1,oo,btarget[ii],0,1)
            ...
            if number_files<nstocks*minfiles:
                for f in datalist:
                    os.remove(os.path.join(outdir + species, f))
            else:
                oo=oo+1
